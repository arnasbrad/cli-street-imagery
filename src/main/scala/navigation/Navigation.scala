...
trait Navigation {
  def findPossibleNavigationOptions(
      ...
      radius: Radius = Radius.unsafeCreate(5),
      maxAmount: Int = 5
  )(implicit
...
object Navigation {
  ...
  case object DistanceBased extends NavigationType

  def make(navigationType: NavigationType): Navigation = {
    navigationType match {
      case DistanceBased => new DistanceBasedNavigation
    }
  }

  // given coordinates, find the closest surrounding images
  private class DistanceBasedNavigation extends Navigation {
    def findPossibleNavigationOptions(
        currentImageId: MapillaryImageId,
        currentCoordinates: Coordinates,
        radius: Radius,
